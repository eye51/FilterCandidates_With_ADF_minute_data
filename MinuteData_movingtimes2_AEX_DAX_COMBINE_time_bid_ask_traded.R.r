...

{
    ...
    title = test$method
#    description = date()
    description =  summary(res)
    new("fURTEST", call = CALL, data = as.data.frame(x), 
        test = test, title = as.character(title), description = as.character(description))
}

##############################################################################################
# Combine 2 time series and calculate ADF.
##############################################################################################


BastiaanMin2Series <-function (start,timeseries)
{


	y<-as.vector(timeseries)

	NoElements<-length(y)/2


	x1<-y[1:NoElements]
	x2<-y[(NoElements+1):(2*NoElements)]

	a<-1               # stock 1 always divided bij stock 2
  b<- -abs(start[1])
...
PerStable <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/candidates/percentage_stable.txt",sep="")
...
cat ("",file=PerStable,sep="",append=FALSE)
cat ("",file=filenameOverzichCandidates,sep="",append=FALSE)

cIndexNames <- scan (file="C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/index/Index.txt",what=character(0),sep="\n",quiet=TRUE)

counter<-0
iNumberOfStocks=(length(cIndexNames)-1)

print (iNumberOfStocks)
SpreadData<-numeric(0)
...

{
    ...
    print (i)
        ...
        filename <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/index/",cIndexNames[i],".txt",sep="")
        ...
        for (j in (i+1):iNumberOfStocks)
            ...
            {
             stock2<-cIndexNames[j]
             filename2 <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/index/",cIndexNames[j],".txt",sep="")
			...
			{
			   ratio=StockData1[((k-1)*NumberOfTicks+1)]/StockData2[((k-1)*NumberOfTicks+1)]
         testData=StockData1[((k-1)*NumberOfTicks+1):(k*NumberOfTicks)]/StockData2[((k-1)*NumberOfTicks+1):(k*NumberOfTicks)]

         SpreadData<-c(SpreadData,testData/ratio)


#				  suppressWarnings(min1<-nlm(BastiaanMin2Series,start,timeseries=combi))
         suppressWarnings(hh<-BastiaanADF (testData,"ct",1))
     ...
     print ("percentage stable")
     print (NoDaysStable/NumberOfDays1)
     cat (stock1,stock2,NoDaysStable/NumberOfDays1,"\n",file=PerStable,sep=",",append=TRUE)
			if (NoDaysStable>(0.6*NumberOfDays1))

                      {                                   # satify all stability criteria..
                            print ("stable")
                            candidate1=strsplit (cIndexNames[i]," EQUITY")
                            candidate2=strsplit (cIndexNames[j]," EQUITY")
                            ...
                            # write combination in single file
                            ...
                            cat (SpreadData,file=filenameCandidate,sep=",")
                            
                            # add combindation to one file which will contain all combindations
                            cat (candidate1[[1]],"_",candidate2[[1]],",",file=filenameAllCandidates,sep="",append=TRUE)
                            cat (SpreadData,file=filenameAllCandidates,sep=",",append=TRUE)
