...
cIndexNames <- scan (file="C:/R-programms/Bastiaan_projects/FilterCandidates_with_ADF/Russell1000+2000_22-12-2004/Index.txt",what=character(0),sep="\n",quiet=TRUE)
...

{
    ...
    print (iNumberOfStocks)
       ...
       filename <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_with_ADF/Russell1000+2000_22-12-2004/",cIndexNames[i],".txt",sep="")

#       filename <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_with_ADF/LLY ln equity.txt",sep="")
        ...
        if ((Capitalization1 >= 500) && (AvgStocks1>100000))
        {
            ...
            
            {
             ...
             filename2 <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_with_ADF/Russell1000+2000_22-12-2004/",cIndexNames[j],".txt",sep="")

                x2 <- scan(file=filename2,quiet=TRUE)                                                                 
                
                NumberObs2=length(x2)-4
                IC2=x2[3]                                         # Bloomberg Indusry code
                Capitalization2=x2[5]                                # cap. of company, use only > 500
                AvgStocks2=x2[6]       
                ...
                if ((Capitalization2 >=500) && (NumberObs1==NumberObs2) && (AvgStocks2>100000)
                ...
                testData=LogStocData1[1:length(LogStocData2)]-LogStocData2[1:length(LogStocData2)];
                ...
                if (stat < crit)
                {
                    suppressWarnings(hh2<-BastiaanADF (testData[(0.5*length(testData)):length(testData)],"c",1))

        	       crit<- -3.3
            	 stat<-hh2@test$statistic
                   if (stat < crit)
                  {

#                       suppressWarnings(hh3<-BastiaanADF (testData[(0.75*length(testData)):length(testData)],"c",1))
#
#	        	       crit<- -3.3
#     	      	 stat<-hh3@test$statistic
#                       if (stat < crit)
...
#
                            ...
                            filenameCandidate=paste("C:/R-programms/Bastiaan_projects/FilterCandidates_with_ADF/candidates/",candidate1[[1]],"_",candidate2[[1]],".txt",sep="")
                            ...
                            cat (attr (hh,"teststat"),file=filenameOverzichCandidates,sep=",",append=TRUE)
                            cat (",2nd=,",file=filenameOverzichCandidates,sep="",append=TRUE)
                            cat (attr (hh2,"teststat"),file=filenameOverzichCandidates,sep=",",append=TRUE)
#                            cat (",3rd=,",file=filenameOverzichCandidates,sep="",append=TRUE)
#                            cat (attr (hh3,"teststat"),file=filenameOverzichCandidates,sep=",",append=TRUE)
                            cat (",",candidate1[[1]],"_",candidate2[[1]],",",file=filenameOverzichCandidates,sep="",append=TRUE)
