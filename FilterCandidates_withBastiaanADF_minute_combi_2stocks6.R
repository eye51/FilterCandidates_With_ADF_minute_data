...
BastiaanMin2Series <-function (start,timeseries)
{
	...
	if (a < 0) noemer=noemer-a*x1 else noemer=noemer+a*x1
	teller=teller+x2
...
cIndexNames <- scan (file="C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/combi/Index.txt",what=character(0),sep="\n",quiet=TRUE)
...
#for (i in 1:(iNumberOfStocks-1))	# just a test if all files contain data...
#{
#	print (i)
#	  stock1 <- paste(cIndexNames[i])
#	  filename <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/combi/",cIndexNames[i],".txt",sep="")
#			  
...

{
	...
	LogStocData1<-log(x1)				  # use rendement for determining stationarity
		...
		filename2 <- paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/combi/",cIndexNames[j],".txt",sep="")

		input2<-read.table (filename2 ,sep=",",header=FALSE) # format date,best bid, best ask, traded
		x2<-input2[[4]]	# traded price


		NumberObs2=length(x2)
		LogStocData2<-log(x2)				   # use rendement for determining stationarity
		...
		
		{
			...
			if ((stat1 <crit))
				...
				if (a < 0) noemer1=noemer1-a*x1 else teller1=teller1+a*x1
				noemer1=noemer1-x2 
				testData1<-teller1/noemer1


				print ("all 3 stable")
				print  (min1$estimate[1])
				print  (min1$iterations[1])
...
####						Write time series in single file			####
					...
					filenameCandidate=paste("C:/R-programms/Bastiaan_projects/FilterCandidates_With_ADF_minute_data/candidates/",candidate1[[1]],"_",candidate2[[1]],"1.txt",sep="")
